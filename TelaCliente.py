...
class CadastroCliente(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(frame, text="Voltar para Clientes", width=200, height=40, fg_color="#6c757d", command=self.voltar).pack()
    ...
    def criar_navbar(self):
        ...
        for texto, cmd in botoes:
            ctk.CTkButton(navbar, text=texto, command=cmd, fg_color="transparent",
...
class ConsultarCliente(CadastroCliente):
    def __init__(self):
        ...
        ctk.CTkButton(frame, text="Voltar para Clientes", width=200, height=40,
...
class ModificarCliente(CadastroCliente):
    ...
    def salvar(self, cpf):
        ...
        try:
            cursor = conexao.cursor()
            cursor.execute("UPDATE clientes SET nome=?, telefone=?, email=? WHERE cpf=?", (nome, telefone, email, cpf))
            conexao.commit()
